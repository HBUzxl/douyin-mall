...
api: ## 自动生成 API 代码
	...
	@/bin/sh -c "\
		...
		goctl rpc protoc ${workdir}/pb/product.proto --style=go_zero --proto_path=${workdir}/pb/ \
			--go_out=${workdir}/product --go-grpc_out=${workdir}/product --zrpc_out=${workdir}/product && \
		goctl rpc protoc ${workdir}/pb/order.proto --style=go_zero --proto_path=${workdir}/pb/ \
			--go_out=${workdir}/order --go-grpc_out=${workdir}/order --zrpc_out=${workdir}/order \
		"
	@echo "生成swagger文件并运行"
